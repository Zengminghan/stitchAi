...
class KimiService:
    def __init__(self, api_key, base_url, max_tokens=512):
    ...
    def get_best_response(self, message, file_path=None):
        if file_path:
            try:
                ...
                return "文件未找到，请确保文件已上传。"
            except PermissionError:
                print(f"没有权限读取文件：{file_path}")
                return "没有权限读取文件，请检查文件权限。"
            except Exception as e:
                print(f"读取文件时出错：{e}")
                return "文件读取失败，请重试。"
        ...
        if best_response and len(best_response) < 100:
...
def home():
    ...
    <body>
        ...
        <form action="/upload" method="post" enctype="multipart/form-data">
            ...
            <input type="submit" value="Upload and解读 File">
        </form>
